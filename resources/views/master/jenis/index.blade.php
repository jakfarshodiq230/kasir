            ...
            <form id="addDataForm">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addDataModalLabel">Tambah Data Jenis</h5>
                    </div>
...
$("#datatable-buttons2").DataTable({
    ...
    processing: true,
    ajax: {
        url: "/jenis/data-jenis-all", // Replace with the Laravel route URL that returns JSON
        dataSrc: "data"
    },
    columns: [
        { data: "id", title: "ID" },
        ...
        },
        {
            ...
            render: function(data, type, row) {
                return `
                    <button class="btn btn-sm btn-primary edit-btn" data-id="${row.id}" data-jenis="${row.jenis}">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-danger delete-btn" data-id="${row.id}">
                        <i class="fas fa-trash-alt"></i> Delete
                    </button>
                    <button class="btn btn-sm ${row.status_jenis == 0? 'btn-secondary' : 'btn-success'} toggle-status-btn" data-id="${row.id}" data-status="${ row.status_jenis == 0 ? 1 : 0}">
                        <i class="fas ${row.status_jenis == 0 ? 'fa-ban' : 'fa-check'}"></i> ${row.status_jenis == 0 ? 'Deactivate' : 'Activate'}
                    </button>
                `;
            }
        }
...
$(document).ready(function() {
    ...
    $('#addDataForm').on('submit', function(event) {
        ...
        var url = '/jenis/save-jenis'; // Default to add
        ...
        if (dataId) {
            ...
            url = '/jenis/update-jenis/' + dataId; // Replace with your edit API endpoint
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        $('#addDataModalLabel').text('Edit Data Jenis');
    ...
    $(document).on('click', '.toggle-status-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '/jenis/status-jenis/' + dataId,  // Your endpoint for updating the status
                    type: 'PUT',  // Use PUT for updating data
                    data: {
                        _token: '{{ csrf_token() }}',  // CSRF token for Laravel
                        status_jenis: newStatus,  // The new status to update
                    },
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
