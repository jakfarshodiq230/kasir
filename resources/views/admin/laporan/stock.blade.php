...
<script>
    ...
    if ($.fn.daterangepicker) {
        ...
        $("#reportrange").on("apply.daterangepicker", function (event, picker) {
            ...
            $.ajax({
                url: "/admin-laporan/stock/data", // Replace with your server endpoint
                method: "POST",
                data: {
                    startDate: startDate,
                    endDate: endDate,
                ...
                if (response.success) {
                    ...
                    response.data.forEach((item, index) => {
                        ...
                        datatable.row.add([
                            index + 1,
                            item.barang ? item.barang.nama_produk : "-",
                            formattedDate,
                            item.stock_masuk,
                            item.stock_keluar,
                            item.stock_akhir,
                            item.keterangan_stock_cabang,
                        ]);
                    });
                    datatable.draw();
                } else {
